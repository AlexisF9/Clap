<template>
  <div v-if="tv" class="c-single-movie">
    ...
    <div class="o-container c-single-movie__titles">
      ...
      <h1 class="c-h-3xl-desk c-h-2xl-mob u-text-white c-single-movie__title">{{ tv.name }}</h1>
    ...
    <div class="o-container u-mt-64 u-text-white c-episodes-rating" v-if="seasons">
      <h2 class="c-h-2xl u-mb-24">Notation des épisodes</h2>
      <div class="c-episodes-rating__seasons">
        <div v-for="season in seasons.sort((a: any, b: any) => a.season_number - b.season_number)">
          <p class="c-text-l u-mb-8 u-align-center" v-if="season.episodes.length > 0">S{{ season.season_number }}</p>
          <ul class="c-episodes-rating__episodes-list" v-if="season.episodes.length > 0">
            <li :class="`c-text-m c-episodes-rating__episode ${episode.vote_count > 0 && getClass(Math.round(episode.vote_average * 10) / 10)}`" v-for="episode in season.episodes">
              ...
              <span class="c-text-l u-fw-600" v-if="episode.vote_count > 0">{{ Math.round(episode.vote_average * 10) / 10 }}</span>
            </li>
          </ul>
        </div>
      </div>
...
type Season = {name: string, season_number: number, episodes: { episode_number: number, vote_count: number, vote_average: any, length: number }}
